...
void idle_setup(user_pt_t* idle_user_pt, kernel_stack_pt_t* idle_kstack_pt, UserContext* uctxt) {
  ...
  pcb_t* idle = pcb_init(idle_user_pt, idle_kstack_pt, uctxt); // set up PCB for the first process. ppid = -1 as kernel is first process
...
pcb_t* init_load(char *name, char *args[], UserContext *uctxt) {
  ...
  pcb_t *init_pcb = pcb_init(ipt, kstack_pt, uctxt);
